...
def main(cfg):
    ...
    for epoch in range(cfg.TRAIN.epochs):
        ...
        for i, data in enumerate(prog_bar): # data is ()
            optimizer.zero_grad()
            images, targets = data
        ...
        writer.add_scalar("train_loss", sum(loss_list)/len(loss_list), epoch)
        ...
        writer.add_scalar("test_loss", sum(val_loss_list)/len(val_loss_list), epoch)
        writer.add_scalar("test_mAP", sum(mAP_list)/len(mAP_list), epoch)
        writer.add_scalar("test_mIOU", sum(mIOU_list)/len(mIOU_list), epoch)
        writer.add_scalar("test_acc", sum(acc_list)/len(acc_list), epoch)
        
        # save the best model
        logger.info(f"\nEPOCH loss: {sum(loss_list)/len(loss_list)}")
        if sum(loss_list)/len(loss_list) < min_loss:
            logger.info("model_saved!")
            torch.save(model.state_dict(),  Path(cfg.MODEL.saved_path)/'best_model.pt')
